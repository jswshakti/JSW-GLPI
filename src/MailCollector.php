...
TWIG, ['receivers_error_msg' => sprintf(__s('Receivers in error: %s'), $server_links)]);
    ...
    private function extractValuesFromRefHeader(string $header): ?array
    {
        ...
        $values = [];
        if (
            preg_match($new_pattern, $header, $values) === 1
            || preg_match($old_pattern_1, $header, $values) === 1
            || preg_match($old_pattern_2, $header, $values) === 1
        ) {
            $result = [
                'uuid'     => null,
                'itemtype' => null,
            ...
            if (!empty($values['uuid'])) {
                $result['uuid'] = $values['uuid'];
            }
            if (!empty($values['event'])) {
                $result['event'] = $values['event'];
            }
            if (!empty($values['itemtype'])) {
                $result['itemtype'] = $values['itemtype'];
            }
            if (!empty($values['items_id'])) {
                $result['items_id'] = (int) $values['items_id'];
            }
